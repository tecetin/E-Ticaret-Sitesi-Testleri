...
public class PazaramaSepeteEkle extends ExtentReportp {
    ...
    public void sepeteEkle(int i) throws IOException  {
        ...
        try {
            // Sepete eklendi mi kontrol et ve daha kısa bir timeout ile kontrol yap

            sepeteEklendi = new WebDriverWait(Driver.getDriver(), Duration.ofSeconds(3))
                    .until(ExpectedConditions.visibilityOfElementLocated(By.xpath(sepeteEklendi(i))));
            Assert.assertTrue(sepeteEklendi.isDisplayed(), "Ürün sepete eklenemedi.");
            ...
            try {
                wait.until(ExpectedConditions.visibilityOf(page.detayPenceresiSepeteEkle)).click();
                ...
                extentTest.pass("Ürünün başarılı bir şekilde default seçeneklerle sepete eklendiği doğrulanmıştır.",
                        MediaEntityBuilder.createScreenCaptureFromBase64String(ReusableMethods.WEResmiBase64(page.penceredenSepeteEklendi)).build());

                wait.until(ExpectedConditions.invisibilityOf(page.penceredenSepeteEklendi));
